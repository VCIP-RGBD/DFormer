...
class UPerHead(nn.Module):
    ...
    def __init__(
        ...
        self.psp_modules = PPM(
            pool_scales, self.in_channels[-1], self.channels, norm_layer=norm_layer, align_corners=align_corners
        )
        ...
        for in_channels in self.in_channels[:-1]:  # skip the top layer
            l_conv = nn.Sequential(
                nn.Conv2d(in_channels, self.channels, 1), norm_layer(self.channels), nn.ReLU(inplace=False)
            )
